...
- `src/lib/` — API client (auto-refresh on 401), Socket.IO wrapper, utilities; `event-actions.ts` (isImportant, AI-chat copy prompt, share/permalink helpers); `event-categories.ts` (the 9 categories + legacy-label mapping — read every stored label through `toCurrentCategory()` and render via `eventCategory()`); `deal-terms.ts` (label/value/order rules for `briefing.deal_terms` — Title Case values, snake_case keys; mirrors the API's `app/utils/deal_terms.py`, keep in sync. Both the Deal Terms panel and the AI-chat prompt read terms through `dealTermEntries()`)
...
- Categories are one flat label per update. The backend classifies against a three-tier taxonomy of ~120 specific events and collapses it before publishing; `briefing.taxonomy` carries the leaf slugs for analytics only. **Never render the leaves or the middle tier** — the reader sees one simple category (`src/lib/event-categories.ts`). Events from before August 2026 carry the old 12 labels, so anything that compares or displays a category maps it through `toCurrentCategory()` first
