...
- `src/components/feed/` — FilingCard (flat row, no box: ticker · price · time, then category, then headline), FilingList (divider-separated; earlier pages load on scroll via an IntersectionObserver sentinel, with a visible button as the fallback), FeedToolbar (All/Important + search + wrapping event-type chips + the live/connecting indicator, all aligned to the reading column), UpdateActions (Read the filing / Copy for AI chat / Share — shown only on expanded updates), CatalystsTable ("Key dates"), DealTerms, PriceReactionStrip, CompanyLogo
...
- `src/app/(dashboard)/` — Dashboard pages (nav rail + main)
  ...
  - `/feed` — public live stream of all events in received order; All/Important toggle, event-type chips, and search, no sidebar. Substack/Twitter-style flat list: rows separated by simple dividers, no card boxes. Prices need a session, so signed-out readers see rows without them
