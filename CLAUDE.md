...
- Companies: `GET /companies/?q=...` (search by ticker or name), `GET /companies/search?q=` (typeahead), `GET /companies/:id/bars` (chart OHLCV), `GET /companies/:id/quote` (last price + day change; `change_pct` is null when there's no previous close, `stale: true` when it's the daily-synced price rather than a live one), `GET /companies/quotes?ids=a,b,c` (the same payload for up to 120 companies at once — backs the price on every feed and watchlist row; companies with no price are absent from the map rather than an error)
...
- `src/hooks/` — useAuth, useSocket, useEvents (REST+Socket merge), useWatchlists, useWatchlistInbox (inbox + live unread + bulk actions), useWatchlistSelection (multi-select mode; range logic in `src/lib/selection.ts`), useCompanyEvents (per-company history), usePaneWidth (resizable pane), useBars (chart data), useQuote (one company's price for the pane header), useQuotes (batch prices for a list of company ids — both poll 60s and pause on hidden tabs)
