...
/**
 * "About" + "Analysis": a short description, the key points, and the
 * rule-generated pros and cons. The pros/cons sentences cite the numbers
 * they were built from (see the API's analysis.py) and are never edited here.
 */
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <div className="min-w-0">
        <h3 className="eyebrow mb-2">About</h3>
        {description ? (
        ...
        {facts.length > 0 && (
          <p className="mt-2 text-meta text-ink-faint">{facts.join(" · ")}</p>
        )}
        {analysis.key_points.length > 0 && (
          <>
            <h4 className="eyebrow mt-4 mb-1.5">Key points</h4>
      ...
      <div className="min-w-0">
        <h3 className="eyebrow mb-2">Analysis</h3>
        {hasAnalysis ? (
          ...
          <p className="text-label text-ink-faint">
            Not enough reported history to judge yet.
          </p>
        )}
        <p className="mt-3 text-micro text-ink-faint">
          Pros and cons are generated from the statements below by fixed rules.
          They are a starting point for reading the numbers, not a
          recommendation.
        </p>
      </div>
    </div>
...
function ProsCons({ title, items }: { title: string; items: string[] }) {
  return (
    <div className={cn("rounded-md border border-line-subtle bg-surface p-3")}>
