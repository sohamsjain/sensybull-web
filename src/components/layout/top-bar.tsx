...
export function TopBar({
  ...
  return (
    <header className="h-14 border-b border-slate-200 dark:border-slate-700 flex items-center px-4 gap-4 shrink-0 bg-white dark:bg-slate-900">
      {/* Mobile menu button */}
      {onMobileMenuToggle && (
        <button
      ...
      >
        <button
          ...
          className={`flex px-3 py-1.5 rounded text-xs font-medium transition-colors items-center gap-1.5 ${
            activeCount > 0
              ...
              : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500"
          }`}
        >
          Event Types
          {activeCount > 0 && (
        ...
        {dropdownOpen && (
          <div className="absolute top-full left-0 mt-1 w-64 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden">
            {activeCount > 0 && (
            ...
            <div className="max-h-72 overflow-y-auto py-1">
              {eventTypes.map((type) => (
                <label
                  key={type}
                  className="flex items-center gap-2 px-3 py-1.5 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 cursor-pointer"
                >
                  <input
                    type="checkbox"
                    checked={eventTypeFilter.has(type)}
                    onChange={() => onEventTypeToggle(type)}
                    className="rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-blue-500 focus:ring-0 focus:ring-offset-0 w-3.5 h-3.5"
                  />
